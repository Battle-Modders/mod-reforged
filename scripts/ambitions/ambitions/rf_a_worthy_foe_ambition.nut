this.rf_a_worthy_foe_ambition <- this.inherit("scripts/ambitions/ambition", {
	...
	function create()
	{
		...
		this.m.TooltipText = "Defeat a Champion in battle while this ambition is active.\nWhile pursuing this ambition, Champions are " + ::MSU.Text.colorPositive("+5%") + " more likely to appear.";
		...
		this.m.RewardTooltip = "Permanently increases the chance for Champions to appear by " + ::MSU.Text.colorPositive("1%");
	...
	function onReward()
	{
		this.m.SuccessList.push({
			...
			text = ::MSU.Text.color(::Const.UI.Color.PositiveEventValue, "+1%") + " chance for Champions to appear",
		});
	}
