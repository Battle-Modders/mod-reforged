this.rf_gain_ground_skill <- ::inherit("scripts/skills/skill", {
	...
	function getTooltip()
	{
		...
		if (!actor.isPlacedOnMap())
		{
			ret.push({
				...
				text = ::Reforged.Mod.Tooltips.parseString("Costs " + ::MSU.Text.colorGreen(2) + " fewer [Action Points|Concept.ActionPoints] than the movement cost of the starting tile")
			});
			ret.push({
				id = 3,
				type = "text",
				icon = "ui/icons/special.png",
				text = ::Reforged.Mod.Tooltips.parseString("Builds [Fatigue|Concept.Fatigue] equal to the movement cost of the starting tile")
			});
		}
		else
		{
	...
	{
		return _targetTile == this.m.ValidTile && _targetTile.IsEmpty && this.skill.onVerifyTarget(_originTile, _targetTile);
	}
	...
	{
		this.m.ValidTile = _targetEntity.isPlacedOnMap() ? _targetEntity.getTile() : null;
	}
