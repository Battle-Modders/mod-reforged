this.rf_sanguine_curse_skill <- ::inherit("scripts/skills/skill", {
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = ::Reforged.Mod.Tooltips.parseString("The target is inflicted with [$ $|Skill+rf_sanguine_curse_effect]")
		});
		ret.push({
			id = 11,
			type = "text",
		...
		ret.push({
			...
			text = ::Reforged.Mod.Tooltips.parseString("Only a single [$ $|Skill+rf_sanguine_curse_effect] can be active at any time")
		});
		return ret;
	}
