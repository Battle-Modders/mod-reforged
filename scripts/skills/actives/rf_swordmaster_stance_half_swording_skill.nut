this.rf_swordmaster_stance_half_swording_skill <- ::inherit("scripts/skills/actives/rf_swordmaster_stance_abstract_skill", {
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = ::Reforged.Mod.Tooltips.parseString(::MSU.Text.colorNegative("Removes") + " all attack skills from the currently equipped sword and adds the [$ $|Skill+stab] and [$ $|Skill+puncture] skills. The Stab skill deals " + ::MSU.Text.colorNegative("50%") + " reduced damage")
		});
