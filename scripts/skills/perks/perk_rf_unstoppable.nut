this.perk_rf_unstoppable <- ::inherit("scripts/skills/skill", {
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = ::Reforged.Mod.Tooltips.parseString("Will expire upon using [Wait|Concept.Wait] or [$ $|Skill+recover_skill] or getting [$ $,|Skill+stunned_effect] rooted, or [$ $|Skill+staggered_effect] or ending the [turn|Concept.Turn] with more than " + ::Math.floor(this.getContainer().getActor().getActionPointsMax() / 2) + " [Action Points|Concept.ActionPoints] remaining")
		});
