this.rf_rebuke_effect <- ::inherit("scripts/skills/skill", {
	...
	{
		if (!this.willProc(_attacker, _skill) || ::Math.rand(1, 100) > this.getChance())
			return;

		local info = {
			Skill = this.getContainer().getAttackOfOpportunity(), // we know it won't be null because we do a hasZoneOfControl check in willProc
