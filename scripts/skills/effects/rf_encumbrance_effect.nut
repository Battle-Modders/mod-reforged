this.rf_encumbrance_effect <- ::inherit("scripts/skills/skill", {
	...
	function getTooltip()
	{
		...
		local fatigueBuildUp = -this.getFatigueRecoveryModifier(level);
		if (fatigueBuildUp)
		{
			tooltip.push({
				id = 10,
	...
	function getFatigueRecoveryModifier( _encumbranceLevel )
	{
		switch (_encumbranceLevel)
		{
			case 0:	// Not a valid level
	...
	{
		local fatigueBuildUp = -this.getFatigueRecoveryModifier(this.getEncumbranceLevel());
		local actor = this.getContainer().getActor();
		actor.setFatigue(::Math.min(actor.getFatigueMax(), actor.getFatigue() + fatigueBuildUp));
	}
});
