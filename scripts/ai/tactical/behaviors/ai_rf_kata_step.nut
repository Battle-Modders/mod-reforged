this.ai_rf_kata_step <- ::inherit("scripts/ai/tactical/behavior", {
	...
	function selectSkill( _possibleSkills )
	{
		...
		foreach (id in _possibleSkills)
		{
			...
			local ap = skill.getActionPointCost() * 3;
	...
	function onEvaluate( _entity )
	{
		...
		local isBodyguard = bodyguardSkill != null && !::MSU.isNull(bodyguardSkill.getVIP()) && bodyguardSkill.getVIP().isAlive();
		...
		foreach (pathIdx, path in paths)
		{
			...
			if (path.BasePath != null)
			{
				...
				path.AlliesEvaluated = path.BasePath.AlliesEvaluated;
