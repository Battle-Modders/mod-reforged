local parties = [
	...
	},
	{
		...
		function onBeforeSpawnStart()
		{
			...
			young.RatioMin = ::Reforged.Math.lerpXClamp(res, 100, 0.5, 820, 0.2);
			berserker.RatioMax = ::Reforged.Math.lerpXClamp(res, 200, 0.6, 720, 0.33);

			// Chance to exclude Berserker.
			if (::Math.rand(1, 100) < ::Reforged.Math.lerpXClamp(res, 100, 75, 300, 0))
			{
				berserker.HardMax = 0;
			}
			...
			else
			{
				...
				if (::Math.rand(1, 100) <= ::Reforged.Math.lerpXClamp(res, 100, 55, 400, 0))
				{
					warrior.HardMax = 0;
				}
				else
			...
			if (warrior.isValid())
			{
				...
				young.RatioMax = ::Reforged.Math.lerpXClamp(res, 100, 1.0, 820, 0.3);
			}
		}
	},
	{
		...
		function onBeforeSpawnStart()
		{
			...
			if (::Math.rand(1, 100) <= ::Reforged.Math.lerpXClamp(res, 300, 75, 400, 0)) // Drops from 75 at 300 to 0 at 400)
			{
				boss.HardMax = 0;
			}
			else
			...
			if (::Math.rand(1, 100) <= ::Reforged.Math.lerpXClamp(res, 200, 56, 300, 0))
			{
				berserker.HardMax = 0;
			}
			...
			// Warlord is absent
			else if (::Math.rand(1, 100) <= ::Reforged.Math.lerpXClamp(res, 200, 0.6,300, 0))
			{
				warrior.HardMax = 0;
			}
			...
			{
				young.RatioMax = ::Reforged.Math.lerpXClamp(res, 200, 1.0, 720, 0.25); // Drops from 1.0 at 200 to 0.25 at 650
				young.RatioMin = ::Reforged.Math.clamp(::Reforged.Math.lerpX(res, 200, 0.5, 720, 0.25), 0.25, 1.0); // Drops from 0.5 at 200 to 0.25 at 720
			}
			...
			if (warrior.isValid())
			{
				...
				berserker.RatioMax = ::Reforged.Math.lerpXClamp(res, 720, 0.33, 200, 0.6);
			}
		}
	},
	{
		...
		function onBeforeSpawnStart()
		{
			...
			// Fewer Young at higher resources
			young.RatioMax = ::Reforged.Math.lerpXClamp(res, 150, 0.7, 800, 0.5); // Drops from 0.7 at 150 to 0.5 at 800
			young.RatioMin = ::Reforged.Math.lerpXClamp(res, 150, 0.5, 800, 0.25); // Drops from 0.5 at 150 to 0.25 at 800
			...
			else
			{
				...
				berserker.RatioMax = ::Reforged.Math.clamp(::Reforged.Math.lerpXSeq(res, [
					[300, 0.25],
					[600, 0.5],
