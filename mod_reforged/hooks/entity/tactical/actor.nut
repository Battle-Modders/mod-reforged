::Reforged.HooksMod.hook("scripts/entity/tactical/actor", function(q) {
	...
	q.onInit = @(__original) { function onInit()
	{
		...
		this.getSkills().add(::new("scripts/skills/special/rf_damage_type_mitigation"));
	...
	{
		// feat: Characters that the player has not yet discovered no longer produce idle sounds.
		// Once they are discovered they will produce idle sounds, even if not visible.
