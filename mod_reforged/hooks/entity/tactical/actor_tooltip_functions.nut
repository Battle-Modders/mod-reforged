::mods_hookExactClass("entity/tactical/actor", function(o) {
	...
	o.getTooltipAttributesSmall <- function( _startID )
	{
		...
		local gapLength = 6;
		local attributeList = [];
		// this.pushSectionName(attributeList, "Attributes", currentID);
		currentID++;
	...
	o.getTooltipEffects <- function( _startID )
	{
        ...
        if (statusEffects.len() != 0 || ::Reforged.Mod.ModSettings.getSetting("HeaderForEmptyCategories").getValue() == true) this.pushSectionName(effectList, "Effects", currentID);
        currentID++;
	...
	o.getTooltipPerks <- function( _startID )
	{
        ...
        if (perks.len() != 0 || ::Reforged.Mod.ModSettings.getSetting("HeaderForEmptyCategories").getValue() == true) this.pushSectionName(perkList, "Perks", currentID);
        currentID++;

        perks.sort(@(a,b) a.getName() <=> b.getName());
		...
		else
		{
			...
			{
				entryText += underlineFirstCharacter(perk.getName());
				entryText += ", ";
			}
			if (entryText.len() != 0) entryText = entryText.slice(0, -2);
	...
	o.getTooltipItems <- function( _startID )
	{
        ...
        if (mainhandItems.len() != 0 || offhandItems.len() != 0 || accessories.len() != 0 || ::Reforged.Mod.ModSettings.getSetting("HeaderForEmptyCategories").getValue() == true) this.pushSectionName(itemList, "Equipped Items", currentID);
		currentID++;
        ...
        if (bagItems.len() != 0 || ::Reforged.Mod.ModSettings.getSetting("HeaderForEmptyCategories").getValue() == true) this.pushSectionName(itemList, "Items in bag", currentID);
		currentID++;
	...
	o.getGroundItems <- function( _startID )
	{
		local currentID = _startID;
		local itemList = [];
	...
	o.underlineFirstCharacter <- function ( _string )
	{
