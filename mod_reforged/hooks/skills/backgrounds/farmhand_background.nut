::Reforged.HooksMod.hook("scripts/skills/backgrounds/farmhand_background", function(q) {
	...
	q.getTooltip = @(__original) { function getTooltip()
	{
		...
		ret.push({
			...
			text = ::Reforged.Mod.Tooltips.parseString(::MSU.Text.colorPositive("+10%") + " chance to hit with [$ $|Item+pitchfork] and " + ::MSU.Text.colorPositive("+5%") + " chance to hit with [$ $|Item+hooked_blade]")
		});
		return ret;
	}}.getTooltip;
