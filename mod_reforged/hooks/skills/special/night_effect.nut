::Reforged.HooksMod.hook("scripts/skills/special/night_effect", function(q) {
	...
	q.onGetHitFactors = @(__original) function( _skill, _targetTile, _tooltip )
	{
		...
		local malus = this.m.HitChancePerTile * (this.getContainer().getActor().getTile().getDistanceTo(_targetTile) - _skill.m.MinRange);
		if (malus < 0)
		{
			_tooltip.push({
				icon = "ui/tooltips/negative.png",
				text = ::Reforged.Mod.Tooltips.parseString(::MSU.Text.colorizePercentage(malus, {AddSign = false}) + " [Nighttime|Skill+night_effect]")
			});
		}
	}
