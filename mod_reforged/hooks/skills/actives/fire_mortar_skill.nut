::Reforged.HooksMod.hook("scripts/skills/actives/fire_mortar_skill", function(q) {
	...
	q.getTooltip = @() { function getTooltip()
	{
		...
		ret.push({
			...
			text = ::Reforged.Mod.Tooltips.parseString("Will mark a tile and its surrounding tiles for impact for this character\'s next [turn.|Concept.Turn] Upon impact, characters in the tiles take damage and may be [$ $|Skill+shellshocked_effect]")
		});
		ret.push({
			id = 11,
			type = "text",
