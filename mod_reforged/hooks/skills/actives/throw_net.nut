::Reforged.HooksMod.hook("scripts/skills/actives/throw_net", function(q) {
	...
	{
		// Prevent crash from TargetEntity having died due or being rotated to a delayed effect while
		// onNetSpawn was scheduled. Doesn't cause any issue in vanilla because throw_net is never used
		// in such a context. Causes crash in Reforged due to perks which trigger delayed attacks
		// and throw_net on hit e.g. Flail Spinner and Kingfisher at the time of writing this comment.
