::Reforged.HooksMod.hook("scripts/skills/actives/hyena_bite_skill", function(q) {
	...
	q.getTooltip = @() { function getTooltip()
	{
		...
		ret.push({
			...
			text = ::Reforged.Mod.Tooltips.parseString("Inflicts [$ $|Skill+bleeding_effect] when dealing at least " + ::MSU.Text.colorNegative(::Const.Combat.MinDamageToApplyBleeding) + " damage to [Hitpoints|Concept.Hitpoints]")
		});
		return ret;
	}}.getTooltip;
