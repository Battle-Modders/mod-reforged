::Reforged.HooksMod.hook("scripts/skills/actives/unstoppable_charge_skill", function(q) {
	...
	q.applyEffectToTarget = @() function( _user, _target, _targetTile )
	{
		...
		if (::Math.rand(1, 2) == 1)		// Maybe stun, stagger otherwise
		{
			...
			local turnsStunned = targetPoiseSkill.onInstabilityDamageReceived(_user, this, _user.getCurrentProperties().getPoiseDamage());
			if (turnsStunned == 0)
			{
		...
		else	// Always Stagger, maybe knock back
		{
			...
			local damage = this.Math.max(0, ::Math.abs(knockToTile.Level - _targetTile.Level) - 1) * ::Const.Combat.FallingDamage;
			if (damage == 0)
			{
