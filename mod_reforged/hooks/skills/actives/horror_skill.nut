::Reforged.HooksMod.hook("scripts/skills/actives/horror_skill", function(q) {
	...
	q.getTooltip = @() { function getTooltip()
	{
		...
		ret.push({
			...
			type = "text",
			icon = "ui/icons/bravery.png",
			text = ::Reforged.Mod.Tooltips.parseString("The target receives an additional mental [morale check|Concept.Morale] with a " + ::MSU.Text.colorNegative(-5) + " penalty to [Resolve.|Concept.Bravery] If this [morale check|Concept.MoraleCheck] succeeds, the target gains the [$ $|Skill+horrified_effect] effect")
		});
		return ret;
	}}.getTooltip;
});
