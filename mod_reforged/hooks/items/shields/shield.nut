	...
	q.getTooltip = @(__original) { function getTooltip()
	{
		...
		if (this.RF_getDefenseMult() != 1.0)
		{
			ret.push({
				...
				text = ::Reforged.Mod.Tooltips.parseString("Except when using [$ $|Skill+shieldwall], the defense values drop linearly as you build [Fatigue|Concept.Fatigue] up to " + ::MSU.Text.colorizeMult(this.RF_getDefenseMult()) + " at maximum [Fatigue|Concept.Fatigue]")
			});
		}
