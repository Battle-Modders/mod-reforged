			...
			Fluff = "Make them scatter and flee!",
	 		Effects = [{
				...
				Description = [
					...
					"These morale checks now have an additional penalty to [Resolve|Concept.Bravery] of " + ::MSU.Text.colorGreen("20%") + " of your [Resolve|Concept.Resolve].",
				]
 			}],
	 	}),
	},
			...
			Fluff = "Look at this filly fiddler!",
	 		Effects = [{
 				...
 				Description = [
					...
					"When used on an adjacent target, reduces its [Melee Defense|Concept.MeleeDefense] and [Ranged Defense|Concept.RangeDefense] by " + ::MSU.Text.colorGreen("20%") + " of the user's [Resolve|Concept.Bravery]."
				]
 			}]
	 	}),
	},
		...
		Description = ::UPD.getDescription({
	 		...
	 		Effects = [{
 				...
 				Description = [
 					...
 					"Gain " + ::MSU.Text.colorGreen("+1") + " [Reach|Concept.Reach] when using two-handed weapons.",
 					"Shields now also negate the [Reach Advantage|Concept.ReachAdvantage] of the target when attacking."
 				]
 			}],
 			Footer = ::MSU.Text.colorRed("This perk ONLY works with melee non-AOE attacks that have a Base Maximum Range of 1 tile, with the exception of [Lunge|Skill+lunge_skill].")
	 	})
	},
	{
		ID = "perk.indomitable",
...
::MSU.Table.merge(::Const.Strings.PerkDescription, {
 		...
 		Effects = [
 			{
				...
				Description = [
					...
					::MSU.Text.colorGreen("50%") + " increased [Resolve|Concept.Resolve] against negative morale checks."
				]
			},
			{
				Type = ::UPD.EffectType.OneTimeEffect,
	...
	RF_OffhandTraining = ::UPD.getDescription({
 		...
 		Effects = [{
			...
			Description = [
				...
				"Once per [turn|Concept.Turn], switching an offhand item costs no [Action Points|Concept.ActionPoints]. Does not work when switching items with a weight of " + ::MSU.Text.colorRed("10") + " or more. Does not stack with other free item swapping skills e.g. Quick Hands.",
	...
	RF_PatternRecognition = ::UPD.getDescription({
 		...
 		Effects = [{
			Type = ::UPD.EffectType.Passive,
			Description = [
				"Every time an opponent attacks you or you attack an opponent, gain a stacking " + ::MSU.Text.colorGreen("+3") + " [Melee Skill|Concept.MeleeSkill] and [Melee Defense|Concept.MeleeDefense] against that opponent for the remainder of the combat.",
 		...
 		Effects = [
	 		{
				...
				Description = [
					...
					"The first two hits against opponents who act after you in the current [round|Concept.Round] recover " + ::MSU.Text.colorGreen("2") + " [Action Points|Concept.ActionPoints] each. A miss against any target or a hit against a target who acted before you increments the counter, but does not recover any [Action Points|Concept.ActionPoints].",
				]
			}
		]
 	}),
