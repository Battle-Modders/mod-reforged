...
::MSU.Table.merge(::Const.Strings.PerkDescription, {
	...
	RF_EnGarde = ::UPD.getDescription({
		...
		Effects = [{
			Type = ::UPD.EffectType.Passive,
			Description = [
				"The double grip bonus for one-handed swords without [Riposte|Skill+riposte] is not disabled when using an offhand item with less than " + ::MSU.Text.colorNegative("10") + " weight."
