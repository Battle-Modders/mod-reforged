	...
	RF_Bloodbath = ::UPD.getDescription({
 	...
 	RF_Bloodlust = ::UPD.getDescription({
 		...
 		Effects = [{
			Type = ::UPD.EffectType.Passive,
			Description = [
				"After inflicting a fatality, [Resolve|Concept.Bravery] and [Initiative|Concept.Initiative] are increased by " + ::MSU.Text.colorGreen("50%") + " and skills build up " + ::MSU.Text.colorGreen("50%") + " less [Fatigue|Concept.Fatigue]. These effects are reduced by " + ::MSU.Text.colorRed("-25%") + " every [turn|Concept.Turn] until another fatality."
			]
		}]
 	}),
	...
	RF_Finesse = ::UPD.getDescription({
 	...
 	RF_Combo = ::UPD.getDescription({
 		...
 		Effects = [{
			Type = ::UPD.EffectType.Passive,
			Description = [
				"Using any skill that costs [Action Points|Concept.ActionPoints], reduces the [Action Point|Concept.ActionPoints] cost of all other skills by " + ::MSU.Text.colorGreen("1") + " to a minimum cost of 3.",
 	...
 	RF_Ghostlike = ::UPD.getDescription({
 		...
 		Effects = [{
			...
			Description = [
				...
				"When you end your movement adjacent to an enemy, the next attack against that enemy gains " + ::MSU.Text.colorGreen("25%") + " increased damage and " + ::MSU.Text.colorGreen("+20%") + " damage ignoring armor. This bonus expires upon any action other than an attack."
			]
		}]
 	}),
