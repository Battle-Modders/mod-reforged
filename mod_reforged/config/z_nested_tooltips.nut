...
::Reforged.QueueBucket.FirstWorldInit.push(function() {
	...
	::MSU.Table.merge(::Reforged.NestedTooltips.Tooltips.Concept, {
		...
		InjuryThreshold = ::MSU.Class.BasicTooltip("Injury Threshold", ::Reforged.Mod.Tooltips.parseString("If the damage received to [Hitpoints|Concept.Hitpoints] is at least " + ::MSU.Text.colorRed(::Const.Combat.InjuryMinDamage) + " and is greater than a certain percentage of the current [Hitpoints|Concept.Hitpoints], the character receives an [injury|Concept.InjuryTemporary]. This percentage can be modified by certain [perks|Concept.Perk] or traits of both the attacker and the target.\n\nCertain [injuries|Concept.InjuryTemporary] require this percentage to be at least a certain value before they can be inflicted, with heavier [injuries|Concept.InjuryTemporary] requiring a higher percentage.\n\nFor example the threshold for [Cut Arm|Skill+cut_arm_injury] is " + ::MSU.Text.colorRed(getThresholdForInjury("injury/cut_arm_injury") + "%") + " and that of [Split Hand|Skill+split_hand_injury] is " + ::MSU.Text.colorRed(getThresholdForInjury("injury/split_hand_injury") + "%") + ".")),
		...
		ReachAttackAdd = ::MSU.Class.BasicTooltip("Reach on attack", ::Reforged.Mod.Tooltips.parseString(@"The amount of [Reach|Concept.Reach] added when this character attacks another.")),
		ReachDefendAdd = ::MSU.Class.BasicTooltip("Reach on defend", ::Reforged.Mod.Tooltips.parseString(@"The amount of [Reach|Concept.Reach] added when this character is attacked by another.")),
