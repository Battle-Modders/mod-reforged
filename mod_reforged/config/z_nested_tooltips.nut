...
::Reforged.QueueBucket.FirstWorldInit.push(function() {
	...
	::MSU.Table.merge(::Reforged.NestedTooltips.Tooltips.Concept, {
		...
		ChanceToHitHead = ::MSU.Class.BasicTooltip("Chance to hit Head", ::Reforged.Mod.Tooltips.parseString("Determines the likelihood of striking an enemy's head instead of their body. A headshot deals " + ::MSU.Text.colorizeMultWithText(::Const.CharacterProperties.DamageAgainstMult[::Const.BodyPart.Head]) + " [hitpoint|Concept.Hitpoints] damage. The default chance to hit the head is " + ::MSU.Text.colorPositive(::Const.CharacterProperties.HitChance[::Const.BodyPart.Head] + "%") + ", but this can be modified by perks or specific attacks. Some enemies lack a head, resulting in all attacks hitting the body."))
	});
